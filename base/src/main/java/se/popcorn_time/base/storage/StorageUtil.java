...
public class StorageUtil {
    ...
    public static void init(Context context) {
        ...
        String path = Prefs.getPopcornPrefs().get(SettingsPrefs.ROOT_FOLDER_PATH, "");
        if ("".equals(path)) {
            for (StorageMount s : INSTANCE.storageMounts) {
                if (s.primary) {
                    setRootDir(new File(s.dir.getAbsolutePath(), ROOT_FOLDER_NAME));
                    break;
                }
            }
        } else {
...
//            clearDir(INSTANCE.rootDir);
            ...
            Prefs.getSettingsPrefs().put(SettingsPrefs.ROOT_FOLDER_PATH, dir.getAbsolutePath());
    ...
    public static String getRootDirPath() {
        return INSTANCE.rootDir != null ? INSTANCE.rootDir.getAbsolutePath() : null;
    }
