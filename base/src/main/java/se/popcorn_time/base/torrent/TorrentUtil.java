...
public final class TorrentUtil {
    ...
    public static String addTorrent(TorrentService service, DownloadInfo info) throws InterruptedException {
        return addTorrent(service, info.directory.getAbsolutePath(), info.torrentFilePath, info.torrentUrl, info.torrentMagnet);
    }

    public static String addTorrent(TorrentService service, String savePath, String file, String url, String magnet) throws InterruptedException {
...
//        Logger.debug("-----------------------------------------------");
        ...
        long timeout = 200;
        if (isTorrentAdded(service, savePath, file, timeout)) {
            return file;
        }
        timeout = TextUtils.isEmpty(magnet) ? 60000 : 15000;
        if (isTorrentAdded(service, savePath, url, timeout)) {
            return url;
        }
        timeout = 120000;
        if (isTorrentAdded(service, savePath, magnet, timeout)) {
            return magnet;
        }
    ...
    private static boolean isTorrentAdded(TorrentService service, String savePath, String metadataPath, long timeout) throws InterruptedException {
        ...
        long currentTime = 0;
        service.addTorrent(metadataPath, savePath);
        try {
            do {
    ...
    public static FileEntry setFilePriority(TorrentService service, String torrent, String fileName, int priority) {
        ...
        if (files != null && files.size() > 0) {
            ...
            if (TextUtils.isEmpty(fileName)) {
                ...
                for (int i = 0; i < files.size(); i++) {
                    String file_name = files.get(i).path.substring(files.get(i).path.lastIndexOf("/") + 1);
