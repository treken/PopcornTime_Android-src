...
//    }
    ...
    * */
    ...
    public void setProxy(VPN vpn) {
        if (bound) {
            if (vpn != null) {
                torrentService.setProxy(proxy_settings.proxy_type.socks5_pw, vpn.host, vpn.port, vpn.user, vpn.pass);
            } else {
                torrentService.setProxy(proxy_settings.proxy_type.none, "", 0, "", "");
            }
        }
    }
