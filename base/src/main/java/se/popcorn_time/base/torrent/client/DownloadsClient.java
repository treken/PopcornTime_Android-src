...
public class DownloadsClient extends BaseClient {
    ...
    public void downloadsAdd(DownloadInfo info, Subtitles subtitles) {
        if (!bound) {
            return;
        }
        ...
        executorService.execute(new AddToDownloadsThread(context, torrentService, info, subtitles));
    ...
    public void downloadsRetry(DownloadInfo info) {
        ...
        info.state = TorrentState.DOWNLOADING;
        Downloads.update(context, info);
        new Thread(new AddToDownloadsThread(context, torrentService, info, null)).start();
    }
            ...
            if (cursor.getCount() > 0) {
                ...
                do {
                    try {
                        ...
                        Logger.error("downloadsPauseAll", e);
                    }
            ...
            if (cursor.getCount() > 0) {
                ...
                do {
                    try {
                        ...
                        Logger.error("downloadsResumeAll", e);
                    }
                } while (cursor.moveToNext());
            }
            cursor.close();
            ...
            if (cursor.getCount() > 0) {
                ...
                do {
                    try {
                        ...
                        Logger.error("downloadsRemoveAll", e);
                    }
                } while (cursor.moveToNext());
