...
public class WatchTask extends Thread {
    ...
    protected String loadMetadata(TorrentService service, WatchInfo info) throws InterruptedException, WatchException {
        ...
        String torrent = TorrentUtil.addTorrent(service, info.watchDir, info.torrentFilePath, info.torrentUrl, info.torrentMagnet);
    ...
    protected String loadSubtitles(TorrentService service, WatchInfo info, String videoPath) throws InterruptedException {
        if (info.subtitles == null) {
            HttpProviderLoader<Subtitles, SubtitlesProvider> loader = new HttpProviderLoader<>(service.getBaseContext(), info.subtitlesProviders);
            info.subtitles = loader.loadInBackground();
            if (Thread.interrupted()) {
                loader.stopLoading();
                throw new InterruptedException("Load subtitles list was interrupted");
            }
        }
        String url = info.subtitles != null ? info.subtitles.getCurrentUrl() : null;
        if (!TextUtils.isEmpty(url)) {
            try {
                String subPath = SubtitlesUtils.generateSubtitlePath(videoPath, FormatSRT.EXTENSION);
                SubtitlesUtils.load(url, subPath);
                return subPath;
            } catch (IOException | SubtitlesException e) {
...
//        Logger.debug(zzz);
    ...
    private void updateProgress(WatchState state, int total, int value) {
        handler.sendMessage(WatchHandler.UPDATE_PROGRESS, new WatchProgress(state, total, value, service.getTorrentSpeed(torrentFile)));
    }
