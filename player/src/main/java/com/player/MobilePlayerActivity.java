...
import com.player.subtitles.Format;
...
public abstract class MobilePlayerActivity extends AppCompatActivity {

    final String TAG = "pt_mobile";
    ...
    private SubtitlesRenderer subtitlesRenderer = new SubtitlesRenderer();

    protected List<SubtitleItem> subtitleItems = new ArrayList<>();
    protected List<AspectRatioItem> aspectRatioItems = new ArrayList<>();
    private List<AudioItem> audioItems = new ArrayList<>();
    protected SubtitleItem currentSubtitleItem;
...
//        subtitlesSurfaceView = (SurfaceView) findViewById(R.id.subtitles_surface_view);
        ...
        subtitleItems.clear();
        ListItemEntity.addItemToList(subtitleItems, withoutSubtitlesItem);
        ListItemEntity.addItemToList(subtitleItems, customSubtitlesItem);
    ...
    protected void setSubtitlesTrack(String filePath, TextFormatter formatter) {
        try {
            subtitlesRenderer.setSubtitlesTrack(new FormatSRT(), new BufferedReader(new FileReader(filePath)), formatter);
            onChosenSubtitles();
    ...
    protected void updateSubtitles(long timeMillis) {
        subtitlesRenderer.onUpdate(timeMillis);
    }
    ...
    protected boolean writeCurrentSubtitles(Format format, String subtitlesPath) {
        if (subtitlesRenderer.getCaptions() != null && subtitlesRenderer.getCaptions().size() > 0) {
            try {
                format.write(new BufferedWriter(new FileWriter(subtitlesPath)), subtitlesRenderer.getCaptions());
                return true;
            } catch (IOException e) {
                e.printStackTrace();
            }
        ...
        if (videoFile.getParentFile() != null && videoFile.getParentFile().isDirectory()) {
            File[] files = videoFile.getParentFile().listFiles(new FileFilter() {
                ...
                public boolean accept(File pathname) {
                    return pathname.isFile() && pathname.getName().endsWith(FormatSRT.EXTENSION);
                }
            });
            for (int i = 0; i < files.length; i++) {
                ListItemEntity.addItemToList(subtitleItems, new SubtitleItem(files[i].getAbsolutePath(), "Track - " + (i + 1)) {
                    ...
                    public void onItemChosen() {
                        super.onItemChosen();
                        Format format = null;
                        if (getValue().endsWith(FormatSRT.EXTENSION)) {
    ...
    private SubtitleItem withoutSubtitlesItem = new SubtitleItem(SubtitlesUtils.WITHOUT_SUBTITLES) {
        @Override
    ...
    private SubtitleItem customSubtitlesItem = new SubtitleItem(SubtitlesUtils.CUSTOM_SUBTITLES) {
        @Override
        public String getName() {
    ...
    * */
    ...
    private View.OnClickListener subtitlesListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            singleChoiceDialog.show(getSupportFragmentManager(), getString(R.string.subtitles), subtitleItems, currentSubtitleItem);
            restartHideOverlay();
        }
    };
    ...
    private FileChooserDialog.OnChooserListener customSubtitlesDialogListener = new FileChooserDialog.OnChooserListener() {
        ...
        public void onChooserSelected(File file) {
            currentSubtitleItem = customSubtitlesItem;
            loadSubtitles(Uri.fromFile(file).toString());
        }
...
//                    Log.d(TAG, "Auto update time");
    ...
    * */
    ...
    private class LoadSubtitlesTask extends AsyncTask<String, Void, String> {
        ...
        protected String doInBackground(String... params) {
            try {
                String subtitlesPath = SubtitlesUtils.generateSubtitlePath(videoFile.getAbsolutePath(), FormatSRT.EXTENSION);
                SubtitlesUtils.load(params[0], subtitlesPath);
                return subtitlesPath;
            } catch (SubtitlesException | IOException | InterruptedException e) {
                e.printStackTrace();
            }
            return null;
        }
    ...
    protected class SubtitleItem extends ListItemEntity<String> {

        public SubtitleItem(String value) {
            super(value);
        }

        public SubtitleItem(String value, String name) {
            super(value, name);
        }

        @Override
        public void onItemChosen() {
            currentSubtitleItem = this;
        }
    }
    ...
    * */
    ...
    public static void start(Context context, Intent intent, Uri uri) {
